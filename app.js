(() => {
  ...
  const PRODUCTS = [
    ...
    { key:'fromagere', label:'Sauce Fromagère', exact:null, portionG:null, kgPerBag:2.2, nominalKgPerBag:2.5, smallKgPerBag:.875, smallNominalKgPerBag:1, smallStep:.25, prepMidi:4, prepSoir:4, step:.25 }
  ];
  ...
  async function init(){
    ...
    const mb = currentMonthBounds();
    if($('caFrom')) $('caFrom').value = mb.from;
    if($('caTo')) $('caTo').value = mb.to;
  ...
  function bindEvents(){
    ...
    ['caRestaurant','caFrom','caTo'].forEach(id => on(id,'change',() => {
      if(caAccessToken) loadCaArchive();
    }));
    on('btnRefreshCa','click',() => loadCaArchive());
    on('btnCloseCaAccessModal','click', closeCaAccessModal);
    on('btnCaAccessLogin','click', caAccessLogin);
    on('caAccessCode','keydown', e => { if(e.key === 'Enter') caAccessLogin(); });
    on('caFileInput','change', e => { const file=e.target.files && e.target.files[0]; if(file) importCaForecastExcel(file); });
    const caZone=$('caUploadZone');
    if(caZone){
      ['dragenter','dragover'].forEach(evt => caZone.addEventListener(evt,e=>{e.preventDefault();caZone.classList.add('dragover');}));
      ['dragleave','drop'].forEach(evt => caZone.addEventListener(evt,e=>{e.preventDefault();caZone.classList.remove('dragover');}));
      caZone.addEventListener('drop', e=>{ const file=e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0]; if(file) importCaForecastExcel(file); });
    }
  ...
  function switchView(view){
    ...
    if(view === 'ca') loadCaArchive();
  }

  function openCaAccessModal(){
    if(!$('caAccessModal')) return;
    $('caAccessCode').value = '';
    $('caAccessError').textContent = '';
    $('caAccessModal').classList.remove('hidden');
    setTimeout(() => $('caAccessCode').focus(), 50);
  }

  function closeCaAccessModal(){
    if($('caAccessModal')) $('caAccessModal').classList.add('hidden');
  }

  async function caAccessLogin(){
    const code = $('caAccessCode').value;
    if(!code){ $('caAccessError').textContent = 'Code requis.'; return; }
    if(!remoteEnabled){
      $('caAccessError').textContent = 'Connexion Google requise.';
      return;
    }
    try{
      const r = await api({action:'verifyCaAccess', code});
      if(!r.ok || !r.token){ $('caAccessError').textContent = 'Code incorrect.'; return; }
      caAccessToken = r.token;
      closeCaAccessModal();
      switchView('ca');
    }catch(e){ $('caAccessError').textContent = 'Accès refusé.'; }
  }
  ...
  async function adminLogin(){
    ...
    try{
      ...
      adminToken = r.token;
      closeAdminModal();
      const target = pendingAdminView || 'settings';
    ...
    if(window.Worker){
      try{
        return await new Promise((resolve,reject) => {
          const worker = new Worker('excel-worker.js?v=3.2.0');
  ...
  async function recomputeForecast(){
    ...
    for(const p of config.products){
      ...
      let inpulseMidi, inpulseSoir, midiForecast, soirForecast;
      let packageInpulseMidi=null, packageInpulseSoir=null, packageMidi=null, packageSoir=null;

      if(p.key === 'fromagere'){
        const largeKg = num(p.kgPerBag) || 2.2; // rendement utile
        const largeNominalKg = num(p.nominalKgPerBag) || 2.5; // appellation poche
        const smallKg = num(p.smallKgPerBag) || .875; // rendement utile
        const smallNominalKg = num(p.smallNominalKgPerBag) || 1; // appellation sachet
        const smallStep = num(p.smallStep) || .25;
        const baseMidiKg = needKg * num(config.general.midiShare) / 100;
        const baseSoirKg = needKg * num(config.general.soirShare) / 100;
        packageInpulseMidi = fromagerePackagePlan(baseMidiKg, num(p.prepMidi), largeKg, smallKg, smallStep, largeNominalKg, smallNominalKg);
        packageInpulseSoir = fromagerePackagePlan(baseSoirKg, num(p.prepSoir), largeKg, smallKg, smallStep, largeNominalKg, smallNominalKg);
        packageMidi = fromagerePackagePlan(baseMidiKg * trendMidi, num(p.prepMidi), largeKg, smallKg, smallStep, largeNominalKg, smallNominalKg);
        packageSoir = fromagerePackagePlan(baseSoirKg * trendSoir, num(p.prepSoir), largeKg, smallKg, smallStep, largeNominalKg, smallNominalKg);
        inpulseMidi = packageInpulseMidi.totalKg / largeKg;
        inpulseSoir = packageInpulseSoir.totalKg / largeKg;
        midiForecast = packageMidi.totalKg / largeKg;
        soirForecast = packageSoir.totalKg / largeKg;
      }else{
        inpulseMidi = Math.max(num(p.prepMidi), ceilStep(baseMidi, num(p.step) || .25));
        inpulseSoir = Math.max(num(p.prepSoir), ceilStep(baseSoir, num(p.step) || .25));
        midiForecast = Math.max(num(p.prepMidi), ceilStep(baseMidi * trendMidi, num(p.step) || .25));
        soirForecast = Math.max(num(p.prepSoir), ceilStep(baseSoir * trendSoir, num(p.step) || .25));
      }
      ...
      details.push({
        ...
        packageLargeKg:p.key === 'fromagere' ? (num(p.kgPerBag)||2.2) : null,
        packageLargeNominalKg:p.key === 'fromagere' ? (num(p.nominalKgPerBag)||2.5) : null,
        packageSmallKg:p.key === 'fromagere' ? (num(p.smallKgPerBag)||.875) : null,
        packageSmallNominalKg:p.key === 'fromagere' ? (num(p.smallNominalKgPerBag)||1) : null,
        packageSmallStep:p.key === 'fromagere' ? (num(p.smallStep)||.25) : null,
        packageInpulseMidi,
        packageInpulseSoir,
        packageMidi,
        packageSoir,
  ...
  function renderForecast(){
    ...
    for(const d of forecastDay.details){
      ...
      if(d.key === 'fromagere'){
        const largeLabel = fmt(d.packageLargeNominalKg || 2.5,1);
        const smallLabel = fmt(d.packageSmallNominalKg || 1,0);
        const pm = packageMargin(d.packageMidi,d.prepMidi) || {};
        const ps = packageMargin(d.packageSoir,d.prepSoir) || {};
        const rows = [
          {
            label:`Fromagère ${largeLabel} kg`,
            inpulse:fmtQuarter(num(d.packageInpulseMidi?.large)+num(d.packageInpulseSoir?.large)),
            adjusted:fmtQuarter(num(d.packageMidi?.large)+num(d.packageSoir?.large)),
            midi:fmtQuarter(d.packageMidi?.large), soir:fmtQuarter(d.packageSoir?.large),
            prepMidi:fmtQuarter(d.prepMidi), prepSoir:fmtQuarter(d.prepSoir),
            marginMidi:fmtQuarter(pm.large), marginSoir:fmtQuarter(ps.large)
          },
          {
            label:`Fromagère ${smallLabel} kg`,
            inpulse:fmtQuarter(num(d.packageInpulseMidi?.small)+num(d.packageInpulseSoir?.small)),
            adjusted:fmtQuarter(num(d.packageMidi?.small)+num(d.packageSoir?.small)),
            midi:fmtQuarter(d.packageMidi?.small), soir:fmtQuarter(d.packageSoir?.small),
            prepMidi:'—', prepSoir:'—',
            marginMidi:fmtQuarter(pm.small), marginSoir:fmtQuarter(ps.small)
          }
        ];
        rows.forEach((r,idx)=>{
          const tr=document.createElement('tr');
          tr.className = idx===0 ? 'fromagere-large-row' : 'fromagere-small-row';
          tr.innerHTML = `
            <td><strong>${esc(r.label)}</strong></td>
            <td class="forecast-total">${r.inpulse}</td>
            <td class="${trendClass}">${r.adjusted}</td>
            <td class="highlight num-strong">${r.midi}</td>
            <td class="highlight num-strong">${r.soir}</td>
            <td>${r.prepMidi}</td>
            <td>${r.prepSoir}</td>
            <td>${r.marginMidi}</td>
            <td>${r.marginSoir}</td>`;
      ...
      const tr = document.createElement('tr');
      const improvedDisplay = `${fmt(improvedTotal,2)} <small>${delta === 0 ? '' : `(${pct(delta)})`}</small>`;
      tr.innerHTML = `
        <td><strong>${esc(d.label)}</strong></td>
        <td class="forecast-total">${fmt(inpulseTotal,2)}</td>
        <td class="${trendClass}">${improvedDisplay}</td>
  ...
  function renderActualTable(){
    ...
    for(const d of realDay.details || []){
      ...
      if(d.key !== 'fromagere'){
        const launched = isMidi ? (old.prepMidiLaunched ?? (old.prepMidiActual !== undefined ? num(old.prepMidiActual) > 0 : false)) : (old.prepSoirLaunched ?? (old.prepSoirActual !== undefined ? num(old.prepSoirActual) > 0 : false));
        const addEq = num(isMidi ? (old.addMidi ?? old.launchMidi ?? 0) : (old.addSoir ?? old.launchSoir ?? 0));
        const actual = (launched ? plannedPrep : 0) + addEq;
        const forecast = num(isMidi ? d.midiForecast : d.soirForecast);
        const variance = actual-forecast;
        const tr = document.createElement('tr');
        tr.dataset.key = d.key;
        tr.innerHTML = `<td data-label="Produit"><strong>${esc(d.label)}</strong></td><td data-label="Prépa prévue">${fmt(plannedPrep,2)}</td><td data-label="Prépa lancée"><label class="prep-check ${launched ? 'is-checked' : ''}"><input class="prep-service-check" type="checkbox" ${launched ? 'checked' : ''} ${locked ? 'disabled' : ''}><span class="prep-check-box" aria-hidden="true"></span></label></td><td data-label="Ajout pendant service"><span class="input-with-unit"><input class="add-service" type="number" min="0" step="0.25" value="${fmtInput(addEq)}" ${locked ? 'disabled' : ''}><em>sac.</em></span></td><td data-label="Réel total" class="actual-service num-strong">${fmt(actual,2)}</td><td data-label="Prévision ajustée">${fmt(forecast,2)}</td><td data-label="Écart" class="variance ${varianceClass(variance)}">${signed(variance)}</td>`;
      ...
      const plan = isMidi ? d.packageMidi : d.packageSoir;
      const largeNominal = num(d.packageLargeNominalKg) || 2.5;
      const smallNominal = num(d.packageSmallNominalKg) || 1;
      const launched = isMidi ? (old.prepMidiLaunched ?? (old.prepMidiActual !== undefined ? num(old.prepMidiActual) > 0 : false)) : (old.prepSoirLaunched ?? (old.prepSoirActual !== undefined ? num(old.prepSoirActual) > 0 : false));
      const addLarge = num(isMidi ? (old.addLargeMidi ?? 0) : (old.addLargeSoir ?? 0));
      const addSmall = num(isMidi ? (old.addSmallMidi ?? 0) : (old.addSmallSoir ?? 0));
      const actualLarge = (launched ? plannedPrep : 0) + addLarge;
      const actualSmall = addSmall;
      const plannedLarge = num(plan && plan.large);
      const plannedSmall = num(plan && plan.small);
      [{pkg:'large',label:`Fromagère ${fmt(largeNominal,1)} kg`,prep:`${fmt(plannedPrep,0)} × ${fmt(largeNominal,1)} kg`,showCheck:true,checked:launched,add:addLarge,actual:actualLarge,forecast:plannedLarge},{pkg:'small',label:`Fromagère ${fmt(smallNominal,0)} kg`,prep:'—',showCheck:false,checked:false,add:addSmall,actual:actualSmall,forecast:plannedSmall}].forEach(r => {
        const variance=r.actual-r.forecast;
        const tr=document.createElement('tr');
        tr.dataset.key=d.key; tr.dataset.package=r.pkg;
        tr.innerHTML=`<td data-label="Produit"><strong>${esc(r.label)}</strong></td><td data-label="Prépa prévue">${r.prep}</td><td data-label="Prépa lancée">${r.showCheck ? `<label class="prep-check ${r.checked ? 'is-checked' : ''}"><input class="prep-service-check" type="checkbox" ${r.checked ? 'checked' : ''} ${locked ? 'disabled' : ''}><span class="prep-check-box" aria-hidden="true"></span></label>` : '—'}</td><td data-label="Ajout pendant service"><span class="input-with-unit"><input class="add-service" type="number" min="0" step="0.25" value="${fmtInput(r.add)}" ${locked ? 'disabled' : ''}><em>${r.pkg==='large' ? fmt(largeNominal,1)+' kg' : fmt(smallNominal,0)+' kg'}</em></span></td><td data-label="Réel total" class="actual-service num-strong">${fmtQuarter(r.actual)}</td><td data-label="Prévision ajustée">${fmtQuarter(r.forecast)}</td><td data-label="Écart" class="variance ${varianceClass(variance)}">${signedQuarter(variance)}</td>`;
        tr.querySelectorAll('input').forEach(input=>input.addEventListener('input',recalcActuals));
        tr.querySelectorAll('input[type="checkbox"]').forEach(input=>input.addEventListener('change',recalcActuals));
        tb.appendChild(tr);
      });
  ...
  function recalcActuals(){
    if(!realDay || !$('actualTable')) return;
    const rows=$$('#actualTable tbody tr');
    rows.filter(tr=>tr.dataset.key!=='fromagere').forEach(tr => {
      const d=(realDay.details||[]).find(x=>x.key===tr.dataset.key); if(!d) return;
      const isMidi=activeService==='MIDI', plannedPrep=num(isMidi?d.prepMidi:d.prepSoir), check=tr.querySelector('.prep-service-check');
      const actual=(check&&check.checked?plannedPrep:0)+num(tr.querySelector('.add-service').value), forecast=num(isMidi?d.midiForecast:d.soirForecast), variance=actual-forecast;
      if(check) check.closest('.prep-check').classList.toggle('is-checked',check.checked);
      tr.querySelector('.actual-service').textContent=fmt(actual,2); const v=tr.querySelector('.variance'); v.textContent=signed(variance); v.className=`variance ${varianceClass(variance)}`;
    });
    const d=(realDay.details||[]).find(x=>x.key==='fromagere'); if(!d) return;
    const isMidi=activeService==='MIDI', plan=isMidi?d.packageMidi:d.packageSoir, plannedPrep=num(isMidi?d.prepMidi:d.prepSoir);
    rows.filter(tr=>tr.dataset.key==='fromagere').forEach(tr=>{
      const pkg=tr.dataset.package, check=tr.querySelector('.prep-service-check'), add=num(tr.querySelector('.add-service').value);
      if(check) check.closest('.prep-check').classList.toggle('is-checked',check.checked);
      const actual=(pkg==='large'&&check&&check.checked?plannedPrep:0)+add, forecast=pkg==='large'?num(plan&&plan.large):num(plan&&plan.small), variance=actual-forecast;
      tr.querySelector('.actual-service').textContent=fmtQuarter(actual); const v=tr.querySelector('.variance'); v.textContent=signedQuarter(variance); v.className=`variance ${varianceClass(variance)}`;
    });
  }

  function collectActiveServiceActuals(){
    const out={};
    $$('#actualTable tbody tr').filter(tr=>tr.dataset.key!=='fromagere').forEach(tr=>{
      const d=(realDay.details||[]).find(x=>x.key===tr.dataset.key); if(!d) return;
      const check=tr.querySelector('.prep-service-check'), launched=!!(check&&check.checked), input=num(tr.querySelector('.add-service').value), plannedPrep=num(activeService==='MIDI'?d.prepMidi:d.prepSoir);
      out[tr.dataset.key]={prepLaunched:launched,prepActual:launched?plannedPrep:0,add:input,actual:(launched?plannedPrep:0)+input};
    });
    const d=(realDay.details||[]).find(x=>x.key==='fromagere');
    if(d){
      const largeRow=$$('#actualTable tbody tr').find(tr=>tr.dataset.key==='fromagere'&&tr.dataset.package==='large'), smallRow=$$('#actualTable tbody tr').find(tr=>tr.dataset.key==='fromagere'&&tr.dataset.package==='small');
      if(largeRow&&smallRow){
        const check=largeRow.querySelector('.prep-service-check'), launched=!!(check&&check.checked), addLarge=num(largeRow.querySelector('.add-service').value), addSmall=num(smallRow.querySelector('.add-service').value), plannedPrep=num(activeService==='MIDI'?d.prepMidi:d.prepSoir);
        const actualLarge=(launched?plannedPrep:0)+addLarge, actualSmall=addSmall, largeUseful=num(d.packageLargeKg)||2.2, smallUseful=num(d.packageSmallKg)||.875, actualKg=actualLarge*largeUseful+actualSmall*smallUseful;
        out.fromagere={prepLaunched:launched,prepActual:launched?plannedPrep:0,addLarge,addSmall,actualLarge,actualSmall,actualKg,actual:actualKg/largeUseful,add:(addLarge*largeUseful+addSmall*smallUseful)/largeUseful,addKg:addLarge*largeUseful+addSmall*smallUseful};
      }
    }
    return out;
  }
      ...
      if(remoteEnabled && teamAccessToken){
        ...
        for(const d of day.details || []){
          ...
          if(activeService === 'MIDI'){
            x.prepMidiLaunched=v.prepLaunched; x.prepMidiActual=v.prepActual; x.addMidi=v.add; x.actualMidi=v.actual; if(v.addKg!==undefined) x.addKgMidi=v.addKg; if(v.actualKg!==undefined) x.actualKgMidi=v.actualKg; if(v.addLarge!==undefined) x.addLargeMidi=v.addLarge; if(v.addSmall!==undefined) x.addSmallMidi=v.addSmall; if(v.actualLarge!==undefined) x.actualLargeMidi=v.actualLarge; if(v.actualSmall!==undefined) x.actualSmallMidi=v.actualSmall;
          }else{
            x.prepSoirLaunched=v.prepLaunched; x.prepSoirActual=v.prepActual; x.addSoir=v.add; x.actualSoir=v.actual; if(v.addKg!==undefined) x.addKgSoir=v.addKg; if(v.actualKg!==undefined) x.actualKgSoir=v.actualKg; if(v.addLarge!==undefined) x.addLargeSoir=v.addLarge; if(v.addSmall!==undefined) x.addSmallSoir=v.addSmall; if(v.actualLarge!==undefined) x.actualLargeSoir=v.actualLarge; if(v.actualSmall!==undefined) x.actualSmallSoir=v.actualSmall;
          }
  ...
  async function importCaForecastExcel(file){
    if(!adminToken){ pendingAdminView='settings'; openAdminModal(); return; }
    try{
      if($('caImportStatus')) $('caImportStatus').textContent='Lecture du fichier…';
      await ensureXlsxLibrary();
      const data=await file.arrayBuffer();
      const wb=XLSX.read(data,{type:'array',cellDates:true,cellFormula:false,cellStyles:false});
      ...
      if(matrix.length<2) throw new Error('Fichier CA vide.');

      const headers=(matrix[0]||[]).map(x=>norm(x));
      const findHeader=(needles)=>headers.findIndex(h=>needles.some(n=>h.includes(n)));
      let bsmCa=findHeader(['ca prev bsm','ca previsionnel bsm','ca prev. bsm']);
      let arsCa=findHeader(['ca prev ars','ca previsionnel ars','ca prev. ars']);
      const dateCols=headers.map((h,i)=>h.startsWith('date')?i:-1).filter(i=>i>=0);
      const nearestDate=i=>{ const left=dateCols.filter(d=>d<i); return left.length?left[left.length-1]:(dateCols[0]??-1); };
      let bsmDate=nearestDate(bsmCa), arsDate=nearestDate(arsCa);

      // Format officiel CA PREV BS AR : A=DATE, B=CA BSM, E=DATE, F=CA ARS.
      if((bsmCa<0 || arsCa<0 || bsmDate<0 || arsDate<0) && (matrix[0]||[]).length>=6){
        bsmDate=0; bsmCa=1; arsDate=4; arsCa=5;
      }
      if(bsmCa<0 || arsCa<0 || bsmDate<0 || arsDate<0) throw new Error('Colonnes CA BSM / ARS non reconnues.');

      const rows=[];
      const seen=new Set();
      for(let i=1;i<matrix.length;i++){
        const r=matrix[i]||[];
        const db=parseDateValue(r[bsmDate]), da=parseDateValue(r[arsDate]);
        const cb=num(r[bsmCa]), ca=num(r[arsCa]);
        if(db && Number.isFinite(cb) && cb>0){
          const k='BSM|'+db; if(!seen.has(k)){ seen.add(k); rows.push({restaurant:'BSM',date:db,forecastCA:cb,source:file.name}); }
        }
        if(da && Number.isFinite(ca) && ca>0){
          const k='ARS|'+da; if(!seen.has(k)){ seen.add(k); rows.push({restaurant:'ARS',date:da,forecastCA:ca,source:file.name}); }
        }
      }
      if(!rows.length) throw new Error('Aucun CA exploitable trouvé.');

      if(remoteEnabled){
        const r=await api({action:'saveCaForecasts',adminToken,rows});
        if(!r.ok) throw new Error(r.error||'Enregistrement impossible.');
        if($('caImportStatus')) $('caImportStatus').textContent=`${r.bsm||0} dates BSM · ${r.ars||0} dates ARS enregistrées`;
      }else{
        caForecasts={BSM:rows.filter(x=>x.restaurant==='BSM'),ARS:rows.filter(x=>x.restaurant==='ARS')};
        if($('caImportStatus')) $('caImportStatus').textContent=`${caForecasts.BSM.length} dates BSM · ${caForecasts.ARS.length} dates ARS chargées localement`;
      }
      showStatus('CA prévisionnels enregistrés.');
      if($('caFileInput')) $('caFileInput').value='';
    }catch(e){
      if($('caImportStatus')) $('caImportStatus').textContent='';
      showStatus(e.message||'Import CA impossible.',true);
    }
  }

  async function loadCaArchive(){
    if(!caAccessToken) return;
    const restaurant = ($('caRestaurant') && $('caRestaurant').value) || 'BSM';
    const from = $('caFrom') ? $('caFrom').value : '';
    const to = $('caTo') ? $('caTo').value : '';
    try{
      if(remoteEnabled){
        const r=await api({action:'listCaForecasts',caAccessToken,restaurant,from,to});
        if(!r.ok) throw new Error(r.error||'Lecture impossible.');
        caForecasts[restaurant]=Array.isArray(r.rows)?r.rows:[];
      }
      renderCaArchive();
    }catch(e){
      if(String(e.message||'').toLowerCase().includes('accès ca')){
        caAccessToken=null;
        openCaAccessModal();
      }
      showStatus(e.message||'Impossible de charger les CA.',true);
    }
  }

  function renderCaArchive(){
    const table=$('caTable'); if(!table) return;
    const restaurant = ($('caRestaurant') && $('caRestaurant').value) || 'BSM';
    const from=$('caFrom') ? $('caFrom').value : '';
    const to=$('caTo') ? $('caTo').value : '';
    const rows=(caForecasts[restaurant]||[])
      .filter(r=>(!from||r.date>=from)&&(!to||r.date<=to))
      .sort((a,b)=>a.date.localeCompare(b.date));
    const tb=table.querySelector('tbody');
    tb.innerHTML=rows.length
      ? rows.map(r=>`<tr><td>${formatDateFR(r.date)}</td><td class="num-strong">${money(r.forecastCA)}</td></tr>`).join('')
      : '<tr><td colspan="2" class="empty-cell">Aucune donnée sur cette période.</td></tr>';
  }
  ...
  function renderDocuments(){
    ...
    tb.innerHTML = rows.map(d => {
      ...
      if(entryPage === 'team') return `<tr><td data-label="Date">${formatDateFR(d.date)}</td><td data-label="Restaurant">${d.restaurant==='BSM'?'Boulogne-sur-Mer':'Armentières'}</td><td data-label="Statut">${status}</td><td data-label="Responsable MIDI">${esc(m)}</td><td data-label="Responsable SOIR">${esc(so)}</td><td data-label="Feuille PDF">${pf}</td></tr>`;
  ...
  function renderParams(){
    const p = $('paramsPanel');
    let html = `<div class="params-group"><h3>Produits, portions et préparations</h3><div class="table-wrap"><table class="data-table params-table"><thead><tr><th>Produit</th><th>Portion g</th><th>Rendement poche kg</th><th>Appellation poche kg</th><th>Rendement petit kg</th><th>Appellation petit kg</th><th>Pas petit sachet</th><th>Pas sachet</th><th>Prépa matin</th><th>Prépa soir</th></tr></thead><tbody>`;
    for(const x of config.products){
      html += `<tr data-pkey="${x.key}"><td><strong>${esc(x.label)}</strong></td><td><input data-f="portionG" type="number" step="1" value="${x.portionG ?? ''}" ${x.key === 'fromagere' ? 'disabled' : ''}></td><td><input data-f="kgPerBag" type="number" step="0.001" value="${x.kgPerBag}"></td><td><input data-f="nominalKgPerBag" type="number" step="0.1" value="${x.nominalKgPerBag ?? ''}" ${x.key !== 'fromagere' ? 'disabled' : ''}></td><td><input data-f="smallKgPerBag" type="number" step="0.001" value="${x.smallKgPerBag ?? ''}" ${x.key !== 'fromagere' ? 'disabled' : ''}></td><td><input data-f="smallNominalKgPerBag" type="number" step="0.1" value="${x.smallNominalKgPerBag ?? ''}" ${x.key !== 'fromagere' ? 'disabled' : ''}></td><td><input data-f="smallStep" type="number" step="0.25" value="${x.smallStep ?? ''}" ${x.key !== 'fromagere' ? 'disabled' : ''}></td><td><input data-f="step" type="number" step="0.25" value="${x.step}"></td><td><input data-f="prepMidi" type="number" step="0.25" value="${x.prepMidi}"></td><td><input data-f="prepSoir" type="number" step="0.25" value="${x.prepSoir}"></td></tr>`;
    }
  ...
  function buildPdf(day, kind){
    ...
    doc.setFontSize(15);
    doc.text(kind === 'closure' ? 'BILAN DE PRODUCTION' : 'FEUILLE DE PRODUCTION', 51, 20);
    doc.setFontSize(8.5);
    ...
    (day.details || []).forEach(d => {
      if(d.key !== 'fromagere'){
        productionRows.push([d.label, fmt(d.midiForecast,2), fmt(d.soirForecast,2), fmt(num(d.midiForecast)+num(d.soirForecast),2)]);
        return;
      }
      const largeLabel=fmt(d.packageLargeNominalKg || 2.5,1);
      const smallLabel=fmt(d.packageSmallNominalKg || 1,0);
      productionRows.push([
        `Fromagère ${largeLabel} kg`,
        fmtQuarter(d.packageMidi && d.packageMidi.large),
        fmtQuarter(d.packageSoir && d.packageSoir.large),
        fmtQuarter(num(d.packageMidi && d.packageMidi.large)+num(d.packageSoir && d.packageSoir.large))
      ]);
      productionRows.push([
        `Fromagère ${smallLabel} kg`,
        fmtQuarter(d.packageMidi && d.packageMidi.small),
        fmtQuarter(d.packageSoir && d.packageSoir.small),
        fmtQuarter(num(d.packageMidi && d.packageMidi.small)+num(d.packageSoir && d.packageSoir.small))
      ]);
    });
    y = pdfTable(doc, y, ['Produit','MIDI','SOIR','Total'], productionRows, {firstColWide:true, orange, black, soft});
    ...
    y = pdfTable(doc, y,
      ...
      (day.details || []).map(d => {
        ...
        return [
          d.key === 'fromagere' ? `Fromagère ${fmt(d.packageLargeNominalKg || 2.5,1)} kg` : d.label,
          d.key === 'fromagere' ? fmtQuarter(d.prepMidi) : fmt(d.prepMidi,2),
          checkedM === null ? '[ ]' : (checkedM ? '[X]' : '[ ]'),
          d.key === 'fromagere' ? fmtQuarter(d.prepSoir) : fmt(d.prepSoir,2),
    ...
    (day.details || []).forEach(d => {
      const a = actualsForPdf[d.key] || {};
      if(d.key !== 'fromagere'){
        marginRows.push([d.label,fmt(d.marginMidi,2),kind === 'closure' ? fmt(a.actualMidi,2) : '',fmt(d.marginSoir,2),kind === 'closure' ? fmt(a.actualSoir,2) : '']);
        return;
      }
      const pm=packageMargin(d.packageMidi,d.prepMidi) || {};
      const ps=packageMargin(d.packageSoir,d.prepSoir) || {};
      const largeLabel=fmt(d.packageLargeNominalKg || 2.5,1);
      const smallLabel=fmt(d.packageSmallNominalKg || 1,0);
      const realLargeMidi = a.actualLargeMidi !== undefined ? num(a.actualLargeMidi) : num(a.actualMidi);
      const realLargeSoir = a.actualLargeSoir !== undefined ? num(a.actualLargeSoir) : num(a.actualSoir);
      const realSmallMidi = a.actualSmallMidi !== undefined ? num(a.actualSmallMidi) : 0;
      const realSmallSoir = a.actualSmallSoir !== undefined ? num(a.actualSmallSoir) : 0;
      marginRows.push([`Fromagère ${largeLabel} kg`,fmtQuarter(pm.large),kind === 'closure' ? fmtQuarter(realLargeMidi) : '',fmtQuarter(ps.large),kind === 'closure' ? fmtQuarter(realLargeSoir) : '']);
      marginRows.push([`Fromagère ${smallLabel} kg`,fmtQuarter(pm.small),kind === 'closure' ? fmtQuarter(realSmallMidi) : '',fmtQuarter(ps.small),kind === 'closure' ? fmtQuarter(realSmallSoir) : '']);
    });
    y = pdfTable(doc, y, ['Produit','Marge MIDI','Réel total MIDI','Marge SOIR','Réel total SOIR'], marginRows, {firstColWide:true, orange, black, soft});
    ...
    if(kind === 'closure'){
      ...
      y = pdfTable(doc, y,
        ...
        (day.details || []).map(d => {
          ...
          if(d.key === 'fromagere'){
            const kg = num(d.packageLargeKg || 2.2);
            return [d.label,`${fmt(pm*kg,2)} kg`,`${fmt(num(x.addKgMidi ?? am*kg),2)} kg`,`${fmt(tm*kg,2)} kg`,`${fmt(ps*kg,2)} kg`,`${fmt(num(x.addKgSoir ?? as*kg),2)} kg`,`${fmt(ts*kg,2)} kg`,`${fmt((num(tm)+num(ts))*kg,2)} kg`];
          }
  ...
  function fromagerePackagePlan(targetKg, prepLarge=0, largeKg=2.2, smallKg=.875, smallStep=.25, largeNominalKg=2.5, smallNominalKg=1){
    targetKg=Math.max(num(targetKg),num(prepLarge)*num(largeKg));
    largeKg=Math.max(.01,num(largeKg)||2.2); // rendement réellement exploitable
    smallKg=Math.max(.01,num(smallKg)||.875); // rendement réellement exploitable
    largeNominalKg=Math.max(.01,num(largeNominalKg)||2.5); // nom commercial de la poche
    smallNominalKg=Math.max(.01,num(smallNominalKg)||1); // nom commercial du sachet
    smallStep=Math.max(.01,num(smallStep)||.25);
    const floorLarge=Math.max(num(prepLarge),Math.floor((targetKg+1e-9)/largeKg));
    const remain=Math.max(0,targetKg-floorLarge*largeKg);
    let smallUnits=remain>1e-9 ? ceilStep(remain/smallKg,smallStep) : 0;
    let large=floorLarge;
    let totalKg=large*largeKg+smallUnits*smallKg;
    const ceilLarge=Math.max(num(prepLarge),Math.ceil((targetKg-1e-9)/largeKg));
    const ceilKg=ceilLarge*largeKg;
    // N'ouvre une nouvelle grande poche que si elle réduit réellement le surplus utile.
    if(ceilKg>=targetKg-1e-9 && (ceilKg-targetKg)<(totalKg-targetKg)-1e-9){
      large=ceilLarge; smallUnits=0; totalKg=ceilKg;
    }
    return {large,small:smallUnits,largeKg,smallKg,largeNominalKg,smallNominalKg,totalKg,targetKg};
  }

  function packageMargin(plan, prepLarge){
    if(!plan) return null;
    return {...plan,large:Math.max(0,num(plan.large)-num(prepLarge)),small:num(plan.small),totalKg:Math.max(0,(num(plan.large)-num(prepLarge))*num(plan.largeKg)+num(plan.small)*num(plan.smallKg))};
  }
  function packageSum(a,b){
    if(!a && !b) return null;
    const x=a||b, aa=a||{large:0,small:0}, bb=b||{large:0,small:0};
    return {large:num(aa.large)+num(bb.large),small:num(aa.small)+num(bb.small),largeKg:num(x.largeKg)||2.2,smallKg:num(x.smallKg)||.875,largeNominalKg:num(x.largeNominalKg)||2.5,smallNominalKg:num(x.smallNominalKg)||1,totalKg:num(aa.totalKg)+num(bb.totalKg)};
  }
  function packageText(plan){
    if(!plan) return '—';
    const parts=[];
    if(num(plan.large)>0) parts.push(`${fmt(plan.large,0)}×${fmt(plan.largeNominalKg || 2.5,1)} kg`);
    if(num(plan.small)>0) parts.push(`${fmt(plan.small,2).replace(/,00$/,'')}×${fmt(plan.smallNominalKg || 1,0)} kg`);
    return parts.length ? parts.join(' + ') : '0';
  }
  function packageHtml(plan){
    if(!plan) return '—';
    return `<span class="pack-main">${esc(packageText(plan))}</span><small>${fmt(plan.totalKg,2)} kg utiles</small>`;
  }
  function packageTotalDisplay(a,b,equivalent){
    const plan=packageSum(a,b);
    return `<span class="pack-main">${esc(packageText(plan))}</span><small>${fmt(plan ? plan.totalKg : num(equivalent)*2.2,2)} kg utiles</small>`;
  }
  function signedUnit(v,unit){ const n=num(v); return `${n>0?'+':''}${fmt(n,2)} ${unit}`; }
  function fmtQuarter(v){ const n=Math.round(num(v)*4)/4; return fmt(n,2).replace(/,00$/,'').replace(/,50$/,',5'); }
  function signedQuarter(v){ const n=Math.round(num(v)*4)/4; return `${n>0?'+':''}${fmtQuarter(n)}`; }
  ...
  function currentMonthBounds(){ const d=new Date(), y=d.getFullYear(), m=d.getMonth(); return {from:localIso(new Date(y,m,1)),to:localIso(new Date(y,m+1,0))}; }
