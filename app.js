(() => {
  ...
  async function init(){
    ...
    if($('caBsmFrom')) $('caBsmFrom').value = mb.from;
    if($('caBsmTo')) $('caBsmTo').value = mb.to;
    if($('caArsFrom')) $('caArsFrom').value = mb.from;
    if($('caArsTo')) $('caArsTo').value = mb.to;
  ...
  function bindEvents(){
    ...
    ['caBsmFrom','caBsmTo'].forEach(id => on(id,'change',() => renderCaArchive('BSM')));
    ['caArsFrom','caArsTo'].forEach(id => on(id,'change',() => renderCaArchive('ARS')));
    on('btnRefreshCaBSM','click',() => loadCaArchive('BSM'));
    on('btnRefreshCaARS','click',() => loadCaArchive('ARS'));
  ...
  async function requestView(view){
    const protectedView = view === 'settings' || view === 'ca-bsm' || view === 'ca-ars';
    if(protectedView && !adminToken){
  ...
  function switchView(view){
    ...
    if(view === 'ca-bsm') loadCaArchive('BSM');
    if(view === 'ca-ars') loadCaArchive('ARS');
  }
    ...
    if(window.Worker){
      try{
        return await new Promise((resolve,reject) => {
          const worker = new Worker('excel-worker.js?v=2.8.0');
  ...
  async function importCaForecastExcel(file){
    ...
    try{
      ...
      const bsmCa=headers.findIndex(h=>h.includes('ca prev bsm') || h.includes('ca prev. bsm'));
      const arsCa=headers.findIndex(h=>h.includes('ca prev ars') || h.includes('ca prev. ars'));
      ...
      const bsmDate=nearestDate(bsmCa), arsDate=nearestDate(arsCa);
      ...
      for(let i=1;i<matrix.length;i++){
        ...
        if(db && Number.isFinite(cb) && cb>0) rows.push({restaurant:'BSM',date:db,forecastCA:cb,source:file.name});
        if(da && Number.isFinite(ca) && ca>0) rows.push({restaurant:'ARS',date:da,forecastCA:ca,source:file.name});
      }
    ...
    }catch(e){ if($('caImportStatus')) $('caImportStatus').textContent=''; showStatus(e.message||'Import CA impossible.',true); }
  }

  async function loadCaArchive(restaurant){
    if(!adminToken) return;
    const pre=restaurant==='BSM'?'caBsm':'caArs';
    const from=$(pre+'From') ? $(pre+'From').value : '';
    const to=$(pre+'To') ? $(pre+'To').value : '';
    try{
      if(remoteEnabled){
        const r=await api({action:'listCaForecasts',adminToken,restaurant,from,to});
        if(!r.ok) throw new Error(r.error||'Lecture impossible.');
        caForecasts[restaurant]=Array.isArray(r.rows)?r.rows:[];
      }
      renderCaArchive(restaurant);
    }catch(e){ showStatus(e.message||'Impossible de charger les CA.',true); }
  }

  function renderCaArchive(restaurant){
    const pre=restaurant==='BSM'?'caBsm':'caArs';
    const table=$(pre+'Table'); if(!table) return;
    const from=$(pre+'From') ? $(pre+'From').value : '';
    const to=$(pre+'To') ? $(pre+'To').value : '';
    const rows=(caForecasts[restaurant]||[]).filter(r=>(!from||r.date>=from)&&(!to||r.date<=to)).sort((a,b)=>a.date.localeCompare(b.date));
    const tb=table.querySelector('tbody');
    tb.innerHTML=rows.length?rows.map(r=>`<tr><td>${formatDateFR(r.date)}</td><td class="num-strong">${money(r.forecastCA)}</td></tr>`).join(''):'<tr><td colspan="2" class="empty-cell">Aucune donnée sur cette période.</td></tr>';
  }
